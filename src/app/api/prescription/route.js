  ...
  try {
    ...
    if (!tokenNumber || !doctorName || !department || !diagnosis) {
      return NextResponse.json(
        {
          error:
            'tokenNumber, doctorName, department and diagnosis are required',
        },
        { status: 400 }
      );
    }
    ...
    // Mark the queue token as Completed — patient leaves the active queue
    await QueueToken.findByIdAndUpdate(queueToken._id, {
      status: 'Completed',
      stage: 'Completed',
    });
