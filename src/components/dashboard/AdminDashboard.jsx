  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        ...
        // Pending Reports — tokens currently in the Lab stage (awaiting report upload)
        const reportRes = await fetch(`/api/token?stage=Lab`);
        const reportData = await reportRes.json();
        const pendingReports = reportData.tokens ? reportData.tokens.length : 0;

        // Department Load — active (Waiting) tokens per department only
        ...
        // Beds Occupied and Revenue Today (appointments booked today × ₹200)
        const bedsOccupied = '62 / 80';
        const todayRes = await fetch(`/api/token?date=${today}`);
        const todayData = await todayRes.json();
        const appointmentsToday = todayData.tokens
          ? todayData.tokens.length
          : 0;
